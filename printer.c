...
zend_function_entry printer_functions[] = {
    PHP_FE(printer_open, arginfo_printer_open) PHP_FE(printer_close, arginfo_printer_close)
        PHP_FE(printer_write, arginfo_printer_write) PHP_FE(printer_list, arginfo_printer_list) PHP_FE(
            printer_set_option, arginfo_printer_set_option) PHP_FE(printer_get_option, arginfo_printer_get_option)
            PHP_FE(printer_create_dc, arginfo_printer_create_dc) PHP_FE(printer_delete_dc, arginfo_printer_delete_dc)
                PHP_FE(printer_start_doc, arginfo_printer_start_doc) PHP_FE(printer_end_doc, arginfo_printer_end_doc)
                    PHP_FE(printer_start_page, arginfo_printer_start_page) PHP_FE(printer_end_page,
                                                                                  arginfo_printer_end_page)
                        PHP_FE(printer_create_pen, arginfo_printer_create_pen) PHP_FE(printer_delete_pen,
                                                                                      arginfo_printer_delete_pen)
                            PHP_FE(printer_select_pen, arginfo_printer_select_pen) PHP_FE(printer_create_brush,
                                ...
                                PHP_FE(printer_delete_brush, arginfo_printer_delete_brush)
                                    PHP_FE(printer_select_brush, arginfo_printer_select_brush)
                                        PHP_FE(printer_create_font, arginfo_printer_create_font) PHP_FE(
                                            printer_delete_font, arginfo_printer_delete_font)
                                            PHP_FE(printer_select_font, arginfo_printer_select_font) PHP_FE(
                                                printer_logical_fontheight, arginfo_printer_logical_fontheight)
                                                PHP_FE(printer_draw_roundrect, arginfo_printer_draw_roundrect)
                                                    PHP_FE(printer_draw_rectangle, arginfo_printer_draw_rectangle)
                                                        PHP_FE(printer_draw_text, arginfo_printer_draw_text) PHP_FE(
                                                            printer_draw_elipse, arginfo_printer_draw_elipse)
                                                            PHP_FE(printer_draw_line, arginfo_printer_draw_line) PHP_FE(
                                                                printer_draw_chord, arginfo_printer_draw_chord)
...
PHP_FUNCTION(printer_open)
{
	...
	if (OpenPrinter(resource->name, &resource->handle, NULL) != 0) {
		...
		if (DocumentProperties(NULL, resource->handle, resource->name, resource->pi2->pDevMode, NULL, DM_OUT_BUFFER) ==
		    IDOK) {
	...
	if (dest) {
		...
		if (dest->instance) {
			...
			if (!resource->dest->instance) {
				...
				efree(resource->dest);
				cupsFreeDests(num_dests, dests);
				php_error_docref(NULL, E_WARNING, "failed to allocate memory for printer instance [%s]",
				                 resource->name);
				efree(resource->name);
				efree(resource);
				RETURN_FALSE;
...
PHP_FUNCTION(printer_write)
{
	...
	job_id = cupsCreateJob(CUPS_HTTP_DEFAULT, resource->dest->name, resource->title, resource->num_options,
	                       resource->options);
	...
	if (cupsStartDocument(CUPS_HTTP_DEFAULT, resource->dest->name, job_id, resource->title, CUPS_FORMAT_RAW, 1) !=
	    HTTP_CONTINUE) {
...
PHP_FUNCTION(printer_set_option)
{
	...
	case VALID_OPTIONS:
		...
		resource->dmModifiedFields = 0;
		DocumentProperties(NULL, resource->handle, resource->name, resource->pi2->pDevMode, resource->pi2->pDevMode,
		                   DM_IN_BUFFER | DM_OUT_BUFFER);
		SetPrinter(resource->handle, 2, (LPBYTE)resource->pi2, 0);
		SendMessageTimeout(HWND_BROADCAST, WM_DEVMODECHANGE, 0L, (LPARAM)(LPCSTR)resource->name, SMTO_NORMAL, 1000,
		                   NULL);
		break;
	default:
...
PHP_FUNCTION(printer_create_font)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "sllllbbl", &face_param, &face_len, &height, &width, &font_weight,
	                          &italic, &underline, &strikeout, &orientation) == FAILURE) {
		RETURN_THROWS();
	}
...
PHP_FUNCTION(printer_draw_roundrect)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "rllllll", &zres, &ul_x, &ul_y, &lr_x, &lr_y, &width, &height) ==
	    FAILURE) {
		RETURN_THROWS();
	}
...
PHP_FUNCTION(printer_draw_chord)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "rllllllll", &zres, &rec_x, &rec_y, &rec_x1, &rec_y1, &rad_x, &rad_y,
	                          &rad_x1, &rad_y1) == FAILURE) {
		RETURN_THROWS();
	}
...
PHP_FUNCTION(printer_draw_pie)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "rllllllll", &zres, &rec_x, &rec_y, &rec_x1, &rec_y1, &rad1_x, &rad1_y,
	                          &rad2_x, &rad2_y) == FAILURE) {
		RETURN_THROWS();
	}
...
PHP_FUNCTION(printer_draw_bmp)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "rsll|ll", &zres, &filename, &filename_len, &x, &y, &width, &height) ==
	    FAILURE) {
		RETURN_THROWS();
	}
	...
	if (has_size) {
		if (!StretchBlt(resource->dc, (int)x, (int)y, (int)width, (int)height, dummy, 0, 0, bmp_property.bmWidth,
		                bmp_property.bmHeight, SRCCOPY)) {
